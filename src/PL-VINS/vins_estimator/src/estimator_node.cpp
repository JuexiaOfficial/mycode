...
getMeasurements()
{
    ...
    while (true)
    {
        ...
        if (!(imu_buf.back()->header.stamp.toSec() > feature_buf.front()->header.stamp.toSec() + estimator.td)) // 如果imu最新数据的时间戳不大于最旧图像的时间戳，那得等imu数据
        {
        ...
        if (!(imu_buf.front()->header.stamp.toSec() < feature_buf.front()->header.stamp.toSec() + estimator.td)) // 如果imu最老的数据时间戳不小于最旧图像的时间，那得把最老的图像丢弃
        {
        ...
        IMUs.emplace_back(imu_buf.front()); // 为插值用,但是imu_buf不会删除front()
...
void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    con.notify_one(); // 用于唤醒等待在条件变量上的一个线程
    {
        // 对m_state进行加锁操作，防止其他线程同时访问m_state，从而保证线程安全性。
        // 当std::lock_guard对象被销毁时，会自动释放锁。
        // 这种方式可以简化对互斥锁的使用，避免忘记解锁的情况发生。
...
void process()
{
    while (true)
    {
        ...
        std::vector<
            std::pair<std::vector<sensor_msgs::ImuConstPtr>, std::pair<sensor_msgs::PointCloudConstPtr, sensor_msgs::PointCloudConstPtr>>>
            measurements;
        // std::unique_lock 被用来创建一个对象 lk，它在构造时自动锁定 m_buf 互斥锁
        // lk 对象拥有 m_buf 互斥锁的所有权，这意味着在 lk 的作用域内，其他线程无法进入这个作用域，直到 lk 被销毁，互斥锁被释放。
        ...
        {
            auto img_msg = measurement.second.first;
            double dx = 0, dy = 0, dz = 0, rx = 0, ry = 0, rz = 0;

            // 处理imu
            // ROS_WARN("-----------start: process IMU----------");
            // std::cout << "-----------start: process IMU----------" << std::endl;
            for (auto &imu_msg : measurement.first) // 遍历vector<sensor_msgs::ImuConstPtr>
            {
                // send_imu(imu_msg); // 处理imu数据, 预测 pose
                double t = imu_msg->header.stamp.toSec();
                double img_t = img_msg->header.stamp.toSec() + estimator.td; // 无论imu的t怎么变，measurement中的img的t是不变的
                if (t <= img_t)
                {
                    if (current_time < 0)
                        current_time = t;
            ...
            map<int, vector<pair<int, Matrix<double, 7, 1>>>> myImage;
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                myImage[feature_id].emplace_back(camera_id, xyz_uv_velocity);
            }
            ...
            estimator.processImage(myImage, lines, img_msg->header);

            double whole_t = t_s.toc();

            printStatistics(estimator, whole_t); //"非线性"阶段才会print
            ...
            pubOdometry(estimator, header, relocalize_t, relocalize_r); // 非线性阶段才pub
            pubKeyPoses(estimator, header, relocalize_t, relocalize_r);
            pubCameraPose(estimator, header, relocalize_t, relocalize_r); // 非线性阶段才pub
            pubLinesCloud(estimator, header, relocalize_t, relocalize_r); // pl-vins
            pubPointCloud(estimator, header, relocalize_t, relocalize_r);
            pubTF(estimator, header, relocalize_t, relocalize_r); // 非线性阶段才pub
            pubKeyframe(estimator);                               // 非线性阶段且marginalization_flag==0 （MARGIN_OLD=0，MARGIN_SECOND_NEW = 1）才pub
...
#endif
    ROS_WARN("_____ estimator_node has started ! _____");
