...
/**
 *
 * FeatureManager 这个类管理着整个VINS中所有的feature
 * |__ feature  (--list<FeaturePerId>--)  以双向链list表的形式管理着所有以id组织起来的的feature（FeaturePerId类）
 * |                                                                                          |__feature的id: feature_id;
 * |                                                                                          |__这个feature的起始帧: start_frame;
 * |                                                                                          |__这个feature在哪些帧里面出现过: feature_per_frame  {--vector<FeaturePerFrame>--}
 * |                                                                                             (vector的形式组织feature出现过的帧)                         |__ 当前的时间偏差 cur_td;
 * |                                                                                                                                                       |__feature的三维位置 Vector3d point
 * |                                                                                                                                                       |__feature的像素位置 Vector2d uv
 * |                                                                                                                                                       |__feature的速度 Vector2d velocity
 * |
 * |__linefeature  (--list<lineFeaturePerId>--)
 *
 *
 *
 */