...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, double td)
{
    ...
    // 把图像中的feature全部放到 feature 中维护
    for (auto &id_pts : image)
    {
        // feature的属性
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td); // 一个单目相机，索引为0
        ...
        // 不是新点，这个点出现的的帧+1（加入到vector<FeaturePerFrame>中），跟踪次数+1
        else if (it->feature_id == feature_id)
        {
            it->feature_per_frame.push_back(f_per_fra);
            last_track_num++;
    ...
    // 遍历线特征（判断共视关系的时候，并没有用线特征）
    // 把图像中的line全部放到 linefeature 中维护
