...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header, Eigen::Vector3d loop_correct_t, Eigen::Matrix3d loop_correct_r)
{
    // “非线性”阶段才会pub
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        ofstream foutC1("/home/qizhengxun/Desktop/ws/ros_modify_PL_VINS_ws/output/orchard_with_no_loop.txt", ios::app);
...
void pubKeyPoses(const Estimator &estimator, const std_msgs::Header &header, Eigen::Vector3d loop_correct_t, Eigen::Matrix3d loop_correct_r)
{
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header, Eigen::Vector3d loop_correct_t, Eigen::Matrix3d loop_correct_r)
{
...
void pubLinesCloud(const Estimator &estimator, const std_msgs::Header &header, Eigen::Vector3d loop_correct_t, Eigen::Matrix3d loop_correct_r)
{
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header, Eigen::Vector3d loop_correct_t, Eigen::Matrix3d loop_correct_r)
{
    // 处于“初始化”阶段不会发送数据
    if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    // 非线性阶段且marginalization_flag==0 （MARGIN_OLD=0，MARGIN_SECOND_NEW = 1）
