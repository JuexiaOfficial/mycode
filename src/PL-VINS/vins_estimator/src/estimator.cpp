...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::Header &header)
{
    // ROS_DEBUG("new image coming ------------------------------------------");
    // ROS_DEBUG("Adding feature points %lu", image.size());

    ROS_WARN("new image coming ------------------------------------------");
    ROS_WARN("Adding feature points %lu", image.size());

    ROS_WARN("Adding lines %lu", lines.size()); // terminal:lines的数量在60左右，是有线的

    // 当视差较大时，marg 老的关键帧
    if (f_manager.addFeatureCheckParallax(frame_count, image, lines, td)) // 对检测到的特征进行存放处理
        marginalization_flag = MARGIN_OLD;
    else
        // 当视差较小时，比如静止，marg 新的图像帧
        marginalization_flag = MARGIN_SECOND_NEW;
    ...
    ROS_WARN("this frame is--------------------%s", marginalization_flag ? "reject" : "accept");
...
void Estimator::solveOdometry()
{
    ...
    if (solver_flag == NON_LINEAR)
    {
        ...
        onlyLineOpt(); // 三角化以后，优化一把。只优化line
