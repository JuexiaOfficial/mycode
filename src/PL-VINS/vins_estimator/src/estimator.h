...
public:
    ...
    void processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::Header &header);
    ...
    SolverFlag solver_flag;                   // 处于哪个状态
    MarginalizationFlag marginalization_flag; // 边缘化老帧还是新帧

    Vector3d g;
    MatrixXd Ap[2], backup_A;
    VectorXd bp[2], backup_b;

    Matrix3d ric[NUM_OF_CAM]; // c到i的旋转
    Vector3d tic[NUM_OF_CAM]; // c到i的位移
    ...
    IntegrationBase *pre_integrations[(WINDOW_SIZE + 1)]; // 预积分量
    ...
    double para_LineFeature[NUM_OF_F][SIZE_LINE]; // pl-vins
    ...
    map<double, ImageFrame> all_image_frame; // 以时间戳为索引，组织ImageFrame这个类
    IntegrationBase *tmp_pre_integration;    // 上次图像到这次图像之间的积分量
