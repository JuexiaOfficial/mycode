...
//         // prev_pts 是向量容器， status指示了每个点是否跟踪成功了，
//         // reduceVector 是将所有跟踪成功的点放到这个向量容器的前面去， 比如1,0,0,1,1,0变成 1,1,1
...
{
    // ROS_WARN("this is start of readImage of FeatureTracker");
    cv::Mat img;
    cur_time = _cur_time;
    ...
    // std::cout << "cur_pts.size() = " << cur_pts.size() << std::endl;//为什么没有显示在终端呢？
    ...
    // ROS_WARN("this is end of readimage");
}
...
void FeatureTracker::myUndistortedPoints()
{
    cur_un_pts.clear();
    ...
    if (!prev_un_pts_map.empty())
    {
        ...
        for (unsigned int i = 0; i < cur_un_pts.size(); i++)
        {
            if (ids[i] != -1)
            {
                ...
                map_it = prev_un_pts_map.find(ids[i]);
                if (map_it != prev_un_pts_map.end())
                {
                    double vx = (cur_un_pts[i].x - map_it->second.x) / dt;
                    double vy = (cur_un_pts[i].y - map_it->second.y) / dt;
