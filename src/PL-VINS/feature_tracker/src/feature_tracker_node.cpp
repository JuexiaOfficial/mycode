...
double frame_cnt = 0; // 算平均时间用的
double sum_time = 0.0;
double mean_time = 0.0;

double last_image_time = 0; // 用于检测不稳定的视频流
bool init_pub = 0;          // 用于跳过检测第一张图片
...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    {
        ROS_WARN("image discontinue!Please check it out.");
    ...
    {
        // ROS_DEBUG默认不会被打印
        // 设置CMAKE_BUILD_TYPE为Debug来启用调试模式，并确保ROS消息和服务生成的选项中包含调试信息。
        ROS_DEBUG("processing camera %d", i);

        if (i != 1 || !STEREO_TRACK)                                                                              // 单目 或者 非双目,   #### readImage： 这个函数里 做了很多很多事, 提取特征 ####
            trackerData[i].readImage(ptr->image.rowRange(ROW * i, ROW * (i + 1)), img_msg->header.stamp.toSec()); // rowRange(i,j) 取图像的i～j行
        else
        {
...
int main(int argc, char **argv)
{
    ...
    ROS_WARN("_____ feature_tracker_node has started ! _____");
