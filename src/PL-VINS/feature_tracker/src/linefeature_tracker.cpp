...
void LineFeatureTracker::readIntrinsicParameter(const string &calib_file)
{
    ...
    K_ = m_camera->initUndistortRectifyMap(undist_map1_, undist_map2_); // 生成用于图像失真校正的映射表
}
...
void LineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    double min_line_length = 0.1; // 0.125 Line segments shorter than that are rejected
    ...
    if (curframe_->keylsd.size() > 0)
    {
        ...
        {
            if (lsd_matches[i].distance < 30) // 用线段之间的“距离”来作为筛选条件
            {
